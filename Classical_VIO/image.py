...
class ImageProcessor(object):
    ...
    def stareo_callback(self, stereo_msg):
        ...
        if self.is_first_img:
            ...
            t = time.time()
            # Draw results.
            # self.draw_features_stereo(self)
    ...
    def draw_features_stereo(self, feature_msg):

        # raw images (H×W)
        img0 = self.cam0_curr_img_msg.image.copy()
        img1 = self.cam1_curr_img_msg.image.copy()
        H, W = img0.shape[:2]

        # convert to BGR so circles show up
        canvas = np.zeros((H, W*2, 3), dtype=np.uint8)
        canvas[:, :W]     = cv2.cvtColor(img0, cv2.COLOR_GRAY2BGR)
        canvas[:, W:W*2]  = cv2.cvtColor(img1, cv2.COLOR_GRAY2BGR)

        K = self.config.cam0_intrinsics
        distortion_model = self.config.cam0_distortion_model
        distortion_coeffs = self.config.cam0_distortion_coeffs
        fx, fy, cx, cy = K[0], K[1], K[2], K[3]
        # for feature in chain.from_iterable(self.curr_features):
        #     # feature.cam0_point, .cam1_point are in pixel coords already
        #     u0, v0 = feature.cam0_point
        #     u1, v1 = feature.cam1_point
        #     x0, y0 = int(u0), int(v0)
        #     x1, y1 = int(u1 + W), int(v1)
        #     cv2.circle(canvas, (x0, y0), 3, (0,255,0), -1)
        #     cv2.circle(canvas, (x1, y1), 3, (0,255,0), -1)
        #     cv2.line(  canvas, (x0,y0), (x1,y1), (255,0,0), 1)

        for fm in feature_msg.features:
            # normalized to pixel
            x0 = int(fm.u0 * fx + cx)
            y0 = int(fm.v0 * fy + cy)
            x1 = int(fm.u1 * fx + cx + W)   # shift right‐image x by W
            y1 = int(fm.v1 * fy + cy)
            # draw
